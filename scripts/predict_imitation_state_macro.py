"""Decode target-input imitation predictions into a playable macro."""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Turn per-tick target_input_down probabilities into a schema-valid "
            "Geometry Dash macro."
        )
    )
    ...
    source_group.add_argument(
        "--predictions-jsonl",
        type=Path,
        help="state-BC predictions.jsonl to decode",
    )
    source_group.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument("--min-state-run-frames", type=int, default=1)
    parser.add_argument(
        "--event-tick-field",
        choices=("tick", "label_tick"),
        default="label_tick",
    )
    ...
    try:
        config = StateDecoderConfig(
            ...
            min_state_run_frames=args.min_state_run_frames,
            event_tick_field=args.event_tick_field,
            player=args.player,
        )
        predictions = _load_or_predict_rows(args, config=config)
        events = decode_state_predictions(predictions, config=config)
        metrics = state_level_metrics(predictions, events, config=config)
    except (StateDecoderError, OSError, ValueError) as exc:
    ...
    macro = Macro(
        events=events,
        metadata={
            "generated_by": "scripts/predict_imitation_state_macro.py",
            "decoder_config": config.to_dict(),
            "source": _source_metadata(args),
            "kind": "predicted_target_input_down_transitions",
        },
    )
...
def _load_or_predict_rows(
    args: argparse.Namespace,
    *,
    config: StateDecoderConfig,
) -> list[Mapping[str, Any]]:
    if args.predictions_jsonl is not None:
        return load_state_prediction_jsonl(
            args.predictions_jsonl,
            event_tick_field=config.event_tick_field,
        )
...
def _default_output_dir(args: argparse.Namespace) -> Path:
    if args.predictions_jsonl is not None:
        return args.predictions_jsonl.parent / "predicted_state_macro"
    return args.checkpoint.parent / "predicted_state_macro"
